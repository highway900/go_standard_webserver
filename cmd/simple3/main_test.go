...
func TestHandler1(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(Post(requestID(handler1)))
	...
	if header := rr.Header().Get(HeaderRequestID); header != "hello" {
		t.Errorf("Incorrect header value for request ID: %v", header)
	}
...
func TestHandler2(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(requestID(handler2))
